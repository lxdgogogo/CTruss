...
def remove_edges_keep_truss_one_layer(graph: list[set[int]], edges: set[tuple], k: int, remove_edges: set[tuple[int, int]],
                                      edge_truss_number: dict[tuple, int], delete_edges: dict[tuple, int] = None):
    """
    ...
    while len(Q_edge) > 0:
        need_remove_edges = Q_edge.pop()
        x, y = need_remove_edges
        graph[x].remove(y)
        graph[y].remove(x)
        edges.remove(need_remove_edges)
        remove_edges.add(need_remove_edges)
        for z in graph[x].copy() & graph[y].copy():
            ...
            if edge_s2 not in edge_truss_number.keys():
                print(1)
...
def get_truss(graph: list[set[int]], edges: set[tuple], nodes: set[int], query_nodes: list[int], k, remove_edges=None,
    ...
    if remove_edges is None:
        remove_edges = set()
    Q_edge = set()
    edge_support = compute_support(graph)
    ...
    while len(Q_edge) > 0:
        ...
        for z in graph[x].copy() & graph[y].copy():
            edge_s1 = (min(x, z), max(x, z))
            edge_s2 = (min(y, z), max(y, z))
            edge_support[edge_s1] -= 1
            edge_support[edge_s2] -= 1
            if edge_support[edge_s1] < k - 2:
                Q_edge.add(edge_s1)
            if edge_support[edge_s2] < k - 2:
                Q_edge.add(edge_s2)
    # 保证其他顶点都与查询点连通
    dsu = DSU(len(graph))
    for edge in edges:
